...
downloadPaper() {
    ...
    echo "$(tput setaf 6)Fetching PaperMC version list..."

    # Fetch API response and store it — don't pipe directly to jq
    API_RESPONSE=$(curl -sL --connect-timeout 10 --max-time 30 https://api.papermc.io/v2/projects/paper 2>/dev/null)
    ...
    # Check if the response is valid JSON with a versions array
    LATEST_VERSION=$(echo "$API_RESPONSE" | jq -r '.versions[-1]' 2>/dev/null)

    if [ -z "$LATEST_VERSION" ] || [ "$LATEST_VERSION" == "null" ]; then
        echo "$(tput setaf 1)ERROR: PaperMC API returned unexpected response."
        echo "$(tput setaf 3)API response: $(echo "$API_RESPONSE" | head -c 200)"
        exit 1
    fi

    echo "$(tput setaf 6)Latest PaperMC version available: ${LATEST_VERSION}"
    ...
    if [ -z "$MINECRAFT_VERSION" ] || [ "$MINECRAFT_VERSION" == "latest" ]; then
        ...
        VER_EXISTS=$(echo "$API_RESPONSE" | jq -r --arg VERSION "$MINECRAFT_VERSION" '.versions[] | select(. == $VERSION)' 2>/dev/null)
        if [ -n "${VER_EXISTS}" ]; then
            echo "$(tput setaf 2)Version is valid. Using version ${MINECRAFT_VERSION}"
        else
            echo "$(tput setaf 3)Version '${MINECRAFT_VERSION}' not found. Defaulting to latest: ${LATEST_VERSION}"
            MINECRAFT_VERSION=${LATEST_VERSION}
        fi
    fi

    # Get build number
    BUILDS_RESPONSE=$(curl -sL --connect-timeout 10 --max-time 30 "https://api.papermc.io/v2/projects/paper/versions/${MINECRAFT_VERSION}/builds" 2>/dev/null)
    ...
    BUILD_NUMBER=$(echo "$BUILDS_RESPONSE" | jq -r '.builds[-1].build' 2>/dev/null)
    ...
    curl -sL --connect-timeout 10 --max-time 300 -o server.jar "${DOWNLOAD_URL}"
